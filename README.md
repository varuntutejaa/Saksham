...
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&duration=2800&pause=1100&color=149B63&center=true&vCenter=true&width=600&lines=Voice-first+%C2%B7+10+Indian+languages;Real+NSQF+%2B+PM-AJAY+government+data;RAG-powered+policy+Q%26A;Now+on+WhatsApp+too" alt="Typing animation" />
...
qualification**, recommends nearby **PM-AJAY** training programmes, and
answers policy questions — all voice-first, low-bandwidth, and grounded in
real government data, not fabricated placeholders.
...
<td><img src="docs/screenshots/04-speak-orb.png" width="200" alt="Voice agent orb" /><br/><sub>Dynamic voice orb</sub></td>
</tr>
<tr>
...
<td><img src="docs/screenshots/08-programs.png" width="200" alt="Programs list" /><br/><sub>PM-AJAY programmes</sub></td>
...
- **Real speech-to-text** — [Sarvam AI](https://sarvam.ai) as the primary
  STT engine (purpose-built for Indian languages), with automatic fallback
  to Groq's hosted Whisper if a Sarvam call fails, and a deterministic
...
- **Conversation history** — the last 3 conversations (voice-agent or
  single-shot) are resumable from the Speak screen.
...
|---|---|
| 1,283 NSQF qualifications ([nqr.gov.in](https://www.nqr.gov.in)) | `TrainingProgram`'s specific seats/contact-number/batch-date fields — no government source publishes that centrally (confirmed by checking directly) |
...
See `server/prisma/data/README*.md` for exact provenance of every dataset.
...
**Sarvam AI** (`saaras:v3`, primary STT, all 10 languages) → **Groq**
(`whisper-large-v3`, automatic fallback) → deterministic offline mock.
On-device TTS (`expo-speech` / Web Speech API) for spoken replies.

</td></tr>
<tr><td><b>RAG / LLM</b></td><td>

**Groq** (`openai/gpt-oss-120b`) generates answers strictly from retrieved
passages. Retrieval is Postgres full-text search (`tsvector`/`ts_rank`) —
no vector DB or embeddings API needed at this corpus size.
...
(recording + live volume metering) · `expo-linear-gradient` ·
`expo-haptics` · `expo-image-picker` + `expo-image-manipulator` (profile
photo) · `@react-native-async-storage/async-storage` (conversation
history, local session) · `@expo/vector-icons`
...
3. **NSQF + location → PM-AJAY programmes** (`recommend.ts`) — weighted
   score: qualification match (0.45), sector (0.20), district (0.12), state
   (0.08), seats available (0.05), stipend (0.05).
4. **Rationale** (`i18n.ts`) — templated "why this" sentence in the
   beneficiary's language, spoken back with on-device TTS.
5. **Policy questions** (`knowledge.ts` + `rag.ts`) — free-text questions
...
6. Every session, mapping, and recommendation is persisted for the **admin
   dashboard**.
